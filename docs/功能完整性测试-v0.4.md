...
| 测试项 | 吞吐量 (ops/sec) | 测试条件 |
|--------|------------------|---------|
| TaskPool 基础吞吐量 | 1,697 | 1000任务，异步执行器 |
| 优先级队列入队 | 208,371 | 内存存储 |
| 优先级队列出队 | 312,443 | 内存存储 |
| 批量弹出吞吐量 | 499,976 | 批量大小100 |
| Worker (1) | 801 | 单Worker |
| Worker (2) | 853 | 双Worker |
| Worker (4) | 805 | 四Worker |
| Worker (8) | 858 | 八Worker |
| 轻量任务吞吐量 | 1,395 | CPU密集型 |
| IO密集型吞吐量 | 4,671 | IO密集型 |
