...
# 导入自定义的控制台颜色处理模块
from neotask.utils.console_colors import init_console_colors, IS_WINDOWS, HAS_COLORAMA, LevelOnlyColoredFormatter
...
class DailyRotatingFileHandler(logging.Handler):
    ...
    def __init__(self, log_dir: str, name: str = 'NeoTask', max_bytes: int = 10 * 1024 * 1024,
...
class LoggingConfigManager:
    ...
    def get_file_handler_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "formatter": "formatter",
            "filename": f"logs/neotask_{datetime.now().strftime('%Y-%m-%d')}.log",
...
class Logger:
    ...
    def __init__(self, name: str = 'NeoTask', log_dir: Optional[str] = None):
        """
        初始化日志器
    ...
    def _create_file_handler(self, handler_config: Dict[str, Any], fmt: str,
        ...
        if log_dir is None:
            # 尝试从配置获取
            filename = handler_config.get('filename', 'logs/neotask_%Y-%m-%d.log')
